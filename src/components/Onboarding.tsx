...
export default function Onboarding({ onComplete, onBack }: OnboardingProps) {
  ...
  useEffect(() => {
    const handleGlobalEnter = (e: KeyboardEvent) => {
      if (e.key === 'Enter') {
        // Prevent submission if name is blank on Step 0
  ...
  return (
    <div className="min-h-screen bg-soft-white dark:bg-espresso flex flex-col justify-between overflow-hidden">
      {/* Top Header Navigation */}
      <div className="pt-8 px-12 pb-4 border-b border-warm-beige/25 dark:border-white/10 bg-white/40 dark:bg-espresso/45 backdrop-blur-md">
        <div className="max-w-xl mx-auto flex items-center justify-between">
           {onBack && (
             <button 
               onClick={onBack} 
               className="flex items-center gap-1.5 px-4 py-2 bg-espresso hover:bg-caramel text-soft-white transition-all rounded-full font-sans font-bold text-xs uppercase tracking-wider shadow-md hover:scale-105 active:scale-95 duration-200 cursor-pointer"
             >
                <ChevronLeft className="w-4 h-4" />
                Back to Main Menu
             </button>
           )}
           <span className="text-[10px] font-black text-caramel uppercase tracking-[0.2em] font-sans">BIO-METRIC ONBOARDING</span>
        </div>
      </div>
        ...
        >
          <AnimatePresence mode="wait">
            {step === 0 && (
              ...
              >
                <div className="w-32 h-32 bg-[#EAD9C0] dark:bg-white/10 rounded-[3rem] items-center justify-center flex mx-auto mb-6 shadow-xl shadow-latte-cream/20 relative group hover:scale-105 transition-all">
                   <span className="text-6xl animate-bounce select-none">🐈🐾</span>
                </div>
                <h1 className="text-5xl font-display font-black text-[#6B4E31] dark:text-soft-white mb-2 tracking-tight">MyCoffe</h1>
                <p className="text-[10px] font-sans font-black uppercase text-caramel tracking-[0.3em] mb-4">Healthy Caffeine & Lifestyle Companion</p>
                <p className="text-lg text-espresso/50 dark:text-soft-white/45 font-medium mb-10 max-w-md mx-auto leading-relaxed">
                  "Hi there! I'm <strong>Barista Cat</strong>, your cozy wellness companion! Let's calculate your personalized cardiovascular safety limit."
                </p>
                
                <div className="space-y-4">
                  <label className="text-[10px] font-bold text-espresso/40 dark:text-soft-white/45 uppercase tracking-[0.4em]">Your Name</label>
                  <input 
                    type="text"
                    placeholder="Enter your name..."
                    value={data.name}
                    onChange={(e) => setData({ ...data, name: e.target.value })}
                    className="w-full px-10 py-7 rounded-[2.5rem] bg-white dark:bg-espresso/50 border border-warm-beige/30 dark:border-white/10 focus:border-caramel/40 outline-none transition-all font-display font-bold text-2xl text-center text-espresso dark:text-soft-white placeholder:text-espresso/10 dark:placeholder:text-soft-white/20"
                  />
                </div>
              </motion.div>
            )}
                ...
                <div className="space-y-10">
                   <div className="space-y-4">
                      <div className="flex items-center justify-between px-6">
                        <label className="text-[10px] font-bold text-espresso/40 dark:text-soft-white/40 uppercase tracking-widest">Weight (kg)</label>
                        <span className="text-2xl font-display font-black text-espresso dark:text-soft-white">{data.weight}</span>
                      </div>
                      <input 
                        type="range"
                        min="40"
                        ...
                        className="w-full h-1.5 bg-latte-cream dark:bg-white/10 rounded-full appearance-none cursor-pointer accent-coffee-brown"
                      />
                   </div>

                   <div className="space-y-4">
                      <label className="text-[10px] font-bold text-espresso/40 dark:text-soft-white/40 uppercase tracking-widest px-6 mb-4 block">Caffeine Sensitivity</label>
            ...
            {step === 4 && (
              <motion.div 
                ...
                className="text-center"
              >
                <div className="w-48 h-48 rounded-full bg-emerald-500/10 border-4 border-emerald-500/20 flex items-center justify-center mx-auto mb-6 relative">
                   <span className="text-7xl animate-pulse select-none">😸👍</span>
                   <div className="absolute inset-0 border-4 border-[#A8C686] rounded-full border-t-transparent animate-spin duration-[3000ms]" />
                </div>
                
                <h2 className="text-[10px] font-sans font-black text-emerald-600 uppercase tracking-[0.5em] mb-4">Mascot Profile Initialized</h2>
