  ...
  if (logs.length === 0) {
    return (
      <div className="max-w-4xl mx-auto p-6 md:p-12 min-h-[80vh] flex flex-col">
        <button 
          onClick={onBack} 
          className="flex items-center gap-2 group px-5 py-2.5 bg-espresso text-soft-white hover:bg-caramel dark:bg-soft-white dark:text-espresso dark:hover:bg-caramel dark:hover:text-soft-white transition-all rounded-full font-sans font-black text-xs uppercase tracking-widest mb-12 shadow-md hover:scale-105 active:scale-95 duration-200 cursor-pointer w-fit"
        >
        ...
        <div className="flex-1 flex flex-col items-center justify-center text-center">
            ...
            <h2 className="text-4xl font-display font-black text-espresso dark:text-soft-white mb-4">No Insights Yet</h2>
            <p className="text-espresso/40 dark:text-soft-white/45 font-medium max-w-sm leading-relaxed mb-10">
              Log your first drinks to unlock powerful analytics about your caffeine habits and energy levels.
            </p>
            <button 
              onClick={onBack}
              className="px-10 py-5 bg-coffee-brown dark:bg-caramel text-white rounded-[2rem] font-bold transition-all hover:scale-105 cursor-pointer"
            >
              Start Logging
            </button>
        </div>
    ...
    <div className="max-w-6xl mx-auto p-6 md:p-12">
      ...
      <div className="grid lg:grid-cols-12 gap-8 items-start">
        <div className="lg:col-span-8 space-y-8">
          <div className="bg-white dark:bg-espresso/40 p-8 rounded-[3rem] border border-warm-beige/30 dark:border-white/10 shadow-xl overflow-hidden">
            <div className="flex items-center justify-between mb-8 px-4">
              <h3 className="text-xl font-display font-black text-espresso dark:text-soft-white flex items-center gap-3">
              ...
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={chartData}>
                  ...
                  <XAxis 
                    ...
                    axisLine={false} 
                    tickLine={false} 
                    tick={{ fill: profile.theme === 'dark' ? '#FAF9F6' : '#2C1810', opacity: 0.4, fontSize: 10, fontWeight: 700 }}
                    dy={15}
                  />
          ...
          <div className="bg-white dark:bg-espresso/40 p-10 rounded-[3rem] border border-warm-beige/30 dark:border-white/10 shadow-xl">
             <h3 className="text-xl font-display font-black text-espresso dark:text-soft-white mb-8 flex items-center gap-3">
             ...
             <div className="grid gap-6">
                {insights.map((insight, i) => (
                  <motion.div 
                    ...
                    className="flex items-center gap-5 p-6 rounded-3xl bg-latte-cream/20 dark:bg-white/5 border border-warm-beige/10 dark:border-white/5"
                  >
                     <div className="w-12 h-12 bg-white dark:bg-espresso/60 rounded-2xl flex items-center justify-center shadow-sm">
                        {insight.icon}
                     </div>
                     <p className="text-sm font-medium text-espresso/70 dark:text-soft-white/70">{insight.text}</p>
                  </motion.div>
                ))}
             </div>
          </div>
        ...
        <div className="lg:col-span-4 space-y-8">
           ...
           <div className="bg-white dark:bg-espresso/45 p-8 rounded-[3rem] border border-warm-beige/30 dark:border-white/10 shadow-xl">
             <h3 className="text-xl font-display font-black text-espresso dark:text-soft-white mb-8">Most Logged</h3>
             <div className="space-y-6">
                {stats.topCategory !== 'None' ? (
                  <div className="flex items-center gap-5 p-6 rounded-3xl bg-soft-white dark:bg-white/5 border border-warm-beige/20 dark:border-white/5">
                    <div className="w-12 h-12 bg-caramel text-white rounded-2xl flex items-center justify-center shadow-md">
                      <Coffee className="w-6 h-6" />
                    </div>
                    <div>
                      <p className="text-lg font-display font-black text-espresso dark:text-soft-white">{stats.topCategory}</p>
                      <p className="text-[10px] font-bold text-espresso/30 dark:text-soft-white/40 uppercase tracking-widest">Favorite Choice</p>
                    </div>
                  </div>
                ) : (
                  <p className="text-center text-espresso/30 dark:text-soft-white/35 text-sm py-4">No categories yet.</p>
                )}
             </div>
           </div>
        </div>
...
function StatCard({ icon, label, value, subtext, color }: { icon: React.ReactNode, label: string, value: string, subtext: string, color: string }) {
  return (
    <div className="bg-white dark:bg-espresso/40 p-8 rounded-[3rem] border border-warm-beige/30 dark:border-white/10 shadow-xl flex items-center gap-6 group hover:translate-y-[-4px] transition-all duration-500">
      ...
      <div>
        <p className="text-[10px] font-bold text-espresso/30 dark:text-soft-white/40 uppercase tracking-[0.2em] mb-1">{label}</p>
        <p className="text-3xl font-display font-black text-espresso dark:text-soft-white mb-1">{value}</p>
        <p className="text-xs text-espresso/40 dark:text-soft-white/50 font-medium">{subtext}</p>
      </div>
    </div>
  );
}
